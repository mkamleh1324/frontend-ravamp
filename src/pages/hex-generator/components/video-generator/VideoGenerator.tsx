...
import { fetchVideo } from "@/redux/HexGeneratorSlice";
...
const VideoGenerator = ({
  ...
  const handlePromptChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    setAnimationPrompt(event.target.value);
  };

  const handleVideoModelChange = (selected: {
    label: string;
    value: string;
  }) => {
    setVideoModel(selected.value);
  };

  const handleRegenerateVideo = () => {
    dispatch(
      fetchVideo({
        videoModel: videoModel.length
          ? videoModel
          : animationModels.firstValue ?? "",
        sceneIndex,
